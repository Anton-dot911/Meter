...
Status: `cd packages/meter-py && uv run pytest` → 25 passed (23 unit + 2 `@llm`). Faithful mirror of meter-ts: `metered_client()` is a duck-typed proxy over `anthropic.Anthropic` intercepting `messages.create` (non-streaming + `stream=True`), plus `record()`, `pricing.py` (with the same dated-snapshot alias normalization), and `SupabaseTransport`+`JsonlFallback`. Records are dispatched on a background daemon thread so a slow/failing transport never adds latency to — or breaks — the host call (Hard Rule 1). **Streaming usage extraction:** the returned SDK `Stream` is wrapped in `_MeteredStream`, which passes every event through unchanged and accumulates usage as the consumer iterates (`message_start` → model + `input_tokens` + initial `output_tokens`; each `message_delta` → cumulative `output_tokens`), then records exactly once on stream end / early break / error / context-exit; the request id comes from the `request-id` response header (`stream.response`) since a raw `Stream` has no `_request_id`. **TS↔Py shape check** (`tests/test_compat.py`): builds a record through the real wrapper with the canonical example's inputs and (a) asserts its key set + per-field JSON types match `spec/record.example.json` and (b) validates it against `spec/record.schema.json` via `jsonschema` — the Python analogue of the ajv check in `scripts/validate-spec.mjs` — so any drift in key/type/enum fails the test. Deps kept minimal: runtime = `supabase` only (anthropic is a type-only/peer dep, imported solely under `TYPE_CHECKING`); dev = `pytest`, `anthropic`, `jsonschema` (test-only, mirrors the repo's ajv spec validation). `@llm` smoke ran live against `claude-haiku-4-5` (non-streaming + streaming) with `METER_ANTHROPIC_API_KEY` + `SUPABASE_SERVICE_ROLE_KEY`, wrote real `py_smoke` rows and read them back from `llm_calls` (e.g. non-streaming: 14 in / 4 out / cost $0.00003 / 887 ms; streaming: 11 in / 9 out / cost $0.00006 / 692 ms, request id from the stream header).
