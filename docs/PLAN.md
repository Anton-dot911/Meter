...
**T4. Scaffolder integration.** ✅ done — **Etap 0 closed.**
...
Status: both scaffolder templates now depend on real Meter as git subdir dependencies — `py-service` via `[tool.uv.sources]` (`packages/meter-py`), `ts-fullstack/service` via `github:...#path:/packages/meter-ts` with `meter-ts` allowlisted in `pnpm.onlyBuiltDependencies` so its `prepare` build runs on install; the no-op stub is deleted and the llm client wraps the SDK with `metered_client`/`meteredClient` (public `create_llm`/`createLlm` + `structured` unchanged). Scaffolder e2e green (py-service 6/6, ts-fullstack 11/11) in [Project-Scaffolder#7](https://github.com/Anton-dot911/Project-Scaffolder/pull/7). Two changes to Meter were needed to make a subdir install viable and correct, each shipped as its own PR merged to main: (#7) add a `prepare` build to `meter-ts` and prefer `SUPABASE_SERVICE_ROLE_KEY` in both transports (owner-only RLS on `llm_calls` rejects anon inserts); (#8) **ship `spec/prices.json` with both package distributions** (git-subdir installs otherwise couldn't find the price table) **and make a pricing-load failure degrade to `cost_usd = null` instead of dropping the record** (Hard Rule 5), with regression tests in both packages. Acceptance: a freshly generated `py-service` ran its `@llm` smoke and wrote a real row to `llm_calls` — `project=meter-t4-final component=smoke-test model=claude-sonnet-4-6 tokens_in=727 tokens_out=33 cost_usd=0.00268 latency_ms=1881 status=ok request_id=req_011CcntoA6niWY2KZemfW28g`, read back from Supabase.
